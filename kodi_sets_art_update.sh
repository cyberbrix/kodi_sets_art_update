...
do
  ...
  setinfo=$(curl -sG --data-urlencode "page=1" --data-urlencode "language=en-US" --data-urlencode "api_key=$api" --data-urlencode "query=$collnameraw" https://api.themoviedb.org/3/search/collection | jq -r --arg collect "$collnameraw" '.results[] | select (.name==$collect)')
  A=$(echo "$setinfo"  | jq -r .backdrop_path)
  B=$(echo "$setinfo"  | jq -r .poster_path)

#  # Check if collection is found. Skip if not
#  if [[ -z "$setinfo" ]]
#  then
#    #echo "$collnameraw not found on TMDB"
#    A="empty"
#    B="empty"
#    #continue
#  fi
  ...
  # Check if collection is found. Skip if not
  if [[ -z "$setinfo" ]] && [ $posterfix -eq 1 ]
  then
    if [ $posterfix -eq 1 ] || [ $fanartfix -eq 1 ]
    then
      echo "$collnameraw not found on TMDB"
    fi #continue
  fi
